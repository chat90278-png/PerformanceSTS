...
class MainWindow(QMainWindow):
    ...
    def on_excel_index_ready(self, platforms, index, timings):
        ...
        self._streaming_index = False
        self._loading = False
        self._store_loading = True   # ExcelStore arka planda yükleniyor
        self._index_ready_for_use = True
        # Loading overlay kapat — liste artık görünür
        self.set_loading_state(False)
        self.update_connection_badge("loading")
        self.connection_label.setText("Liste hazır · Düzenleme modu arka planda hazırlanıyor…")
    ...
    def on_excel_loaded(self, store, index):
        ...
        if self.store:
            ...
            self.update_connection_badge("bad")
            self.connection_label.setText("Excel düzenleme modu başarısız")
        # Platform listesi index_ready'de zaten kuruldu; sadece takvimi güncelle
        self.refresh_open_calendar()
        if self._pending_select_platform:
            self.select_platform(self._pending_select_platform)
    ...
    def on_excel_load_progress(self, percent: int, message: str):
        ...
        if self._loading and hasattr(self, "loading_label"):
            ...
            if self._index_ready_for_use:
                # Mesajı doğrudan göster (ping canlı günceller)
                self.update_connection_badge("loading")
    ...
    def _ensure_store(self) -> bool:
        """
        ExcelStore hazır mı kontrol eder.
        Hazırsa True döner.
        ...
        if getattr(self, "_store_loading", False):
            QMessageBox.information(
                self,
                "Hazırlanıyor",
                "Düzenleme modu arka planda hazırlanıyor.\n\n"
                "Büyük Excel dosyaları için bu birkaç dakika sürebilir.\n"
                "Bağlantı göstergesi ✓ yeşil olduğunda tekrar deneyin.",
            )
            return False
        # Store yüklenemedi (hata) - yeniden dene
        if not self.path or not self.path.exists():
            QMessageBox.information(self, "Excel gerekli", "Önce bir Excel dosyası bağlayın.")
            return False
        # Son çare: anlık yükle (normalde buraya gelinmemeli)
        self.set_busy_overlay(True, "Excel düzenleme modu yükleniyor...", 0)
        try:
            import gc
            from src.services.excel_store import ExcelStore
            gc.collect()
